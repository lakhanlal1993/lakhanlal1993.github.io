...
function renderPublications(data,featured) {
  ...
  $('#publication-list').innerHTML=all.map(x=>{const dest=x.url||(x.doi?'https://doi.org/'+x.doi:'');return '<article class="publication-row"><span class="pub-year">'+(x.year?esc(x.year):'In review')+'</span><div><h3>'+esc(x.title)+'</h3><p>'+esc(x.journal)+' · '+esc(x.status||x.type)+(Number.isFinite(x.citations)?' · '+esc(x.citations)+' citations':'')+'</p></div><div class="pub-link">'+(dest?'<a '+linkAttrs(dest)+'>'+(x.doi?'DOI: '+esc(x.doi):'Publisher page')+' ↗</a>':'')+'</div></article>';}).join('');
}
